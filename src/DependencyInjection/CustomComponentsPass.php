        ...
        foreach ($container->getParameter('.daisyui_twig_components.custom_components') as $key => $attributes) {
            ...
            if ($parent === []) {
                throw new RuntimeException(sprintf(
                    'Unable to infer the parent for custom component "%s". Ensure the name ends with one of: %s.',
                    $key,
                    implode(', ', array_keys($parentsMap))
                ));
            }
            ...
            if ($parentKey === $key) {
                throw new RuntimeException(sprintf(
                    'Custom component "%s" has the same name as its parent, which would result in overriding it.',
                    $key
                ));
            }
